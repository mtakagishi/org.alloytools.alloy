...
Alloy4は、jar形式の実行可能ファイルです。
jarには以下のようなものが包含されています。
* Kodkod model finder
* 複数のSATソルバー
* 標準Alloyライブラリ
* Alloyライブラリの利用例
...
最新の機能詳細はリリースノートを参照してください。

ドキュメント: http://alloytools.org/documentation.html.

# 動作環境

Alloyは、最近のJVM（Java 6以降）上で動作します。
JVM上で動作するため、クロスプラットフォームのSATソルバーとして活用可能です。
([Sat4j](http://www.sat4j.org/),[minisat](http://minisat.se), [lingeling/plingeling](http://fmv.jku.at/lingeling/), [glucose](http://www.labri.fr/perso/lsimon/glucose/)).

Note）macOS High Sierra以降では、Alloy動作のために専用のJVMが必要です。対策用の`.pkg`ファイルが提供されています。
